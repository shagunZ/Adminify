  ...
  return (
    <div className='ml-3 h-screen md:overflow-hidden overflow-auto md:hover:overflow-auto pb-10'>
      ...
      <div className='flex justify-between items-center'>
        <Link to="/" onClick={handleCloseSideBar} className='items-center gap-3 ml-3 mt-5 flex text-xl font-extrabold tracking-tight dark:text-white text-slate-900'>
          <BiBasket /><span>Basket</span>
        </Link>
        <TooltipComponent content="Menu" position='BottomCenter'/>
        <button type='button'
        onClick={()=>setActiveMenu(!activeLink)} 
        className='text-xl rounded-md p-3 hover:bg-light-gray mt-3 block ' >
          <MdOutlineCancel/>
        </button>
      </div>
